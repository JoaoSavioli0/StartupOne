...
export default function NewBookingBoxComponent() {
  ...
  const [title, setTitle] = useState("Escolha o espaço");
  ...
  function stepOneNext(newStep: number, selectedSpaceId: number, newTitle: string) {
    setStep(newStep);
  ...
  function stepTwoNext(newStep: number, newTitle: string) {
    setStep(newStep);
    setTitle(newTitle);
  }
  ...
  return (
    <div>
      <button onClick={() => setVisible(true)}>
        <SelectionButton title="Agendar espaço" icon="calendar" />
      </button>
      <Dialog
        header={title}
        visible={visible}
      ...
      >
        {step == 2 &&
          <button
            onClick={() => stepTwoNext(3, "Agendamento")}
        ...
        <div className="w-full min-h-[150px] h-full relative">
            ...
            <div className="flex flex-col gap-y-0.5">
              {availableSpaces.map((info) => (
                <button key={info.id} className="w-full cursor-pointer h-max bg-gray-100 hover:bg-gray-200 transition-colors duration-100 flex p-3 justify-between items-center first:rounded-t-md last:rounded-b-md"
                  onClick={() => {
                    stepOneNext(2, info.id, "Verifique a disponibilidade");
                  }}>
              ...
              <div className="flex w-full overflow-hidden mt-3" ref={emblaRef}>
                <div className="flex w-full gap-x-2">
                  {dates.map((date) => {
                    ...
                    return (
                      <button key={date.toISOString()} onClick={() => setSelectedDate(date)} className={`px-2 py-0.5 text-sm rounded-full  cursor-pointer ${isSelected ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-600'}`}><span className="font-light">{dayName}</span><span className="ml-1 font-medium">{dayNumber}</span></button>
                    )
                  })}
                </div>
              </div>
          ...
          {step == 3 && (
            <div className="w-full h-full relative">
              <p className="font-semibold">Agendando {selectedPlace?.name} para o dia {selectedDate.getDay().toString()} de {monthYear}</p>
